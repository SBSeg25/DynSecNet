...
wget "https://github.com/SBSeg25/DynSecNet/raw/refs/heads/main/app/docs/contrib/netbox-postgresql.tar.gz" -O netbox-postgresql.tar.gz
...
docker run -d --network netbox-docker_default --name=grafana -p 3000:3000 greentopcookie/dynsecnet:grafana
echo -e '\e[33mBaixando contêiner e iniciando atacante\e[0m'
docker run -d --network netbox-docker_default --name=ubuntu-rogue greentopcookie/dynsecnet:rogue
echo -e '\e[36m******************\e[0m'
